...
Caio Cordeiro Fabri <email@ufmg.br> XX%
Rafael Sant'Ana <email@ufmg.br> XX%
...
int fork1(void) {
    ...
    */
    ...
    if (pid < 0) {
        // Se pid for menor que 0, ocorreu um erro na criação do processo
        fprintf(stderr, "Erro: falha na chamada fork()\n");
        return -1;
    }

    // Retorna o PID:
    // - Para o processo pai, será o PID do filho (> 0)
    // - Para o processo filho, será 0
    return pid;
    /* END OF TASK 1 */
}
...
void handle_simple_cmd(struct execcmd *ecmd) {
    ...
    // ATENÇÃO: A função execvp SÓ RETORNA se algo der errado!
    // Se ela funcionar, o processo atual vira o novo programa (ex: ls)
    // e o código abaixo nunca será alcançado.
...
void handle_redirection(struct redircmd *rcmd) {
    ...
    // Fecha o file descriptor alvo (0 para '<' ou 1 para '>')
    close(rcmd->fd);
    
    // Abre o arquivo especificado. Como acabamos de fechar o rcmd->fd, 
    // o open() vai obrigatoriamente reutilizar esse número.
    // O 0666 garante permissões básicas de leitura/escrita caso o arquivo precise ser criado.
...
void handle_pipe(struct pipecmd *pcmd, int *p, int r) {
    ...
    // 1. Cria o pipe. p[0] será leitura, p[1] será escrita.
    if (pipe(p) < 0) {
        fprintf(stderr, "Erro ao criar o pipe\n");
        exit(-1);
    }

    // 2. Cria o processo filho da ESQUERDA do pipe (ex: ls)
    if (fork1() == 0) {
        close(1);      // Fecha o stdout padrão do terminal
        dup(p[1]);     // Duplica a ponta de escrita do pipe para o menor FD livre (agora é o 1)
        
        // Fecha as pontas originais do pipe, pois o filho já as clonou
        close(p[0]);
        close(p[1]);
        
        runcmd(pcmd->left); // Executa o comando da esquerda
    }

    // 3. Cria o processo filho da DIREITA do pipe (ex: wc)
    if (fork1() == 0) {
        close(0);      // Fecha o stdin padrão do teclado
        dup(p[0]);     // Duplica a ponta de leitura do pipe para o menor FD livre (agora é o 0)
    ...
    // 4. No processo pai (o shell), fechamos as pontas originais do pipe
    close(p[0]);
    close(p[1]);

    // 5. Esperamos os dois filhos (esquerda e direita) terminarem antes de continuar
    wait(&r);
    wait(&r);
...
int main(void) {
    ...
    while (getcmd(buf, sizeof(buf)) >= 0) {
        ...
        /* Answer:
           O comando 'cd' deve ser executado pelo próprio processo do shell (built-in).
           Se utilizássemos fork() e exec(), a mudança de diretório ocorreria apenas no 
           processo filho, que seria destruído logo em seguida, deixando o shell pai no 
           diretório original. 
           
           A mensagem original está incorreta porque a função chdir() manipula caminhos e 
           diretórios, não processos. A falha ocorre se o diretório não existir ou se não 
           houver permissão de acesso. A nova mensagem reflete o erro real do sistema.
         */
        if (buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' ') {
            buf[strlen(buf) - 1] = 0;
            if (chdir(buf + 3) < 0)
                // MENSAGEM CORRIGIDA ABAIXO:
                fprintf(stderr, "cd: diretório não existe ou acesso negado\n");
            continue;
        }
        /* END OF TASK 5 */
